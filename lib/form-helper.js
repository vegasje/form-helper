	...
	var _init = function($form, settings) {
		...
		function fillFieldDefaults(fields, parent) {
			$.each(fields, function(name, fieldSettings) {
				...
				if (parent !== undefined) fieldSettings.parent = parent;
				if (!fieldSettings.validate) fieldSettings.validate = [];
				if (!fieldSettings.onFail) fieldSettings.onFail = settings.onFail;
				if (!fieldSettings.onPass) fieldSettings.onPass = settings.onPass;
				if (fieldSettings.fields) fillFieldDefaults(fieldSettings.fields, fieldSettings);
			});
		}
		...
		function buildPublicFields(fields, currentPublicFields) {
			$.each(fields, function(name, fieldSettings) {
				...
				if (fieldSettings.fields) buildPublicFields(fieldSettings.fields, currentPublicFields[name]);
			});
		}
